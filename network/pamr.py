...
class PAMR(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # x_std = self.aff_std(x)
        # aff1 = -self.aff_x(x) / (1e-8 + 0.1 * x_std)
        # aff1 = aff1.mean(1, keepdim=True)
        # if fea is not None:
        #     fea=F.interpolate(fea, size=x.size()[-2:], mode="bilinear", align_corners=True)
        #     fea_std = self.aff_std(fea)
        #     aff2=-self.aff_x(fea) / (1e-8 + 0.1 * fea_std)
        #     aff2=aff2.mean(1, keepdim=True)
        #     x = F.softmax(aff1, 2)+F.softmax(aff2, 2)
        # else:
        #     x = F.softmax(aff1, 2)
