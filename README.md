...
1. Training. 
   ...
   we use the segmentation repo from https://github.com/YudeWang/semantic-segmentation-codebase. Training and inference codes are available in ```segmentation/experiment/```. Set ```DATA_PSEUDO_GT: $your_pseudo_label_path``` in ```config.py```. Here, we provide our pseduo-masks (https://) generated by the single-stage RTC. Then run:
   ```
   python train.py
   ```
