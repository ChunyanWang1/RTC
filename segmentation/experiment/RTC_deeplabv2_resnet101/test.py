...
from utils.DenseCRF import dense_crf, _crf_inference#dense_crf_from_deeplabv2
...
def test_net():
	...
	def collect_func(result_list, sample):
		...
		if cfg.TEST_CRF:
			...
			predict_img=np.argmax(prob, axis=0).astype(np.uint8)
			...
			prob=_crf_inference(img_batched,predict_img)
