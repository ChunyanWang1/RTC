...
class ResultWriter:
    ...
    def __init__(self,  out_path, verbose=True):#cfg, palette,
        #self.cfg = cfg
    ...
    def _crf_with_alpha(self,cam_dict, alpha,orig_img):
        v = cam_dict[1:] #np.array(list(cam_dict.values()))
        bg_score = np.power(1 - np.max(v, axis=0, keepdims=True), alpha)
        bgmask_score = np.concatenate((bg_score, v), axis=0)
        pred_crf = crf_inference(orig_img, bgmask_score, t=10, scale_factor=1, labels=21)
        return pred_crf


    def save(self, img_path, img_orig, all_masks, labels, pads):
        ...
        #print(self.root)
        merged_mask = self._merge_masks(all_masks, labels, pads, img_orig255.shape[:2])
        ...
        # # crf_alpha=[5] #4,24,32
        # # for t in crf_alpha:
        # #     pred_crf = self._crf_with_alpha(merged_mask, t, img_orig255)
        ...
        # folder1 = os.path.join(self.root, "mask")
        # if not os.path.exists(folder1):
        #     os.makedirs(folder1)
        # np.save(os.path.join(folder1, img_name + '.npy'), merged_mask)
...
class MergeMultiScale(ResultWriter):
    ...
    def _merge_masks(self, masks, labels, pads, imsize_hw):
        ...
        mean_mask[0, ::] = 0.25 #best for val-set
        #mean_mask[0, ::] = 0.25 #best for val-clean
